...
export default function SignUpForm() {
  ...
  // Adiciona handler para signup social Google
  const handleGoogleSignUp = async () => {
    try {
      ...
      if (result?.error) {
        setFormError("root", {
          message: result.error.message || "Falha no registro com Google",
        });
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Erro inesperado ao registrar com Google";
      setFormError("root", {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    try {
      ...
      if (result.error) {
        setFormError("root", {
          message: result.error.message || "Falha no registro",
        });
      } else {
        ...
        <div>
          ...
          <div>
            <div className="grid grid-cols-1 gap-3">
              <button
                type="button"
                className="w-full inline-flex items-center justify-center gap-3 py-3 text-sm font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-7 hover:bg-gray-200 hover:text-gray-800 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10"
