...
export default function SignInForm() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      if (result.error) {
        setFormError("root", {
          message: result.error.message || "Falha no login",
        });
      } else {
  ...
  // Adiciona handler para login social Google
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result?.error) {
        setFormError("root", {
          message: result.error.message || "Falha no login com Google",
        });
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Erro inesperado ao logar com Google";
      setFormError("root", {
        message: errorMessage,
      });
    }
